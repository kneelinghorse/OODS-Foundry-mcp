...
const DEFAULT_FILTER_ID = 'interaction-filter';
const DEFAULT_FILTER_EVENT = 'drag';
const DEFAULT_FILTER_ENCODINGS: readonly IntervalChannel[] = ['x'];

export function useFilter(options: UseFilterOptions = {}): InteractionTrait {
  const { id = DEFAULT_FILTER_ID, event = DEFAULT_FILTER_EVENT, encodings = DEFAULT_FILTER_ENCODINGS } = options;

  const encKey = encodings.join('|');
  const normalizedEncodings = useMemo(
    () => toEncodingTuple(encodings, 'useFilter requires at least one encoding channel.'),
    [encKey]
  );
